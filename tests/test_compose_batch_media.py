...
def test_runs_without_capture_are_skipped_and_existing_media_is_kept(tmp_path, monkeypatch):
    ...
    (recorded / "frames.json").write_text('{"frames": [{"index": 0}]}')
    ...
    fresh.mkdir()
    (fresh / "frames.json").write_text('{"frames": [{"index": 0}]}')
    refused = batch / "run_03_source_tree0_v4_fine_step"
    refused.mkdir()
    (refused / "frames.json").write_text('{"frames": []}')
    (refused / "experiment_result.json").write_text("{}")
    ...
    assert results[refused.name]["status"] == "skipped_no_frames"
